package org.bigleg.async.http.spdy;

import org.bigleg.async.AsyncServer;
import org.bigleg.async.AsyncSocket;
import org.bigleg.async.BufferedDataSink;
...
import org.bigleg.async.callback.WritableCallback;
import org.bigleg.async.future.SimpleFuture;
...
//            setIdle(false);
    ...
    void updateWindowRead(int length) {
        ...
        if (totalWindowRead >= okHttpSettings.getInitialWindowSize(Settings.DEFAULT_INITIAL_WINDOW_SIZE) / 2) {
            try {
    ...
    public class SpdySocket implements AsyncSocket {
        ...
        public void addBytesToWriteWindow(long delta) {
            ...
            if (bytesLeftInWriteWindow > 0 && prev <= 0)
                org.bigleg.async.Util.writable(writable);
        }
...
//            return;
        ...
        org.bigleg.async.Util.emitAllData(socket, socket.pending);
        if (inFinished) {
            sockets.remove(streamId);
            socket.close();
            org.bigleg.async.Util.end(socket, null);
        }
    }
...
//            return;
        ...
        if (rstStream != null) {
            org.bigleg.async.Util.end(rstStream, new IOException(errorCode.toString()));
        }
    }
    ...
    void addBytesToWriteWindow(long delta) {
        bytesLeftInWriteWindow += delta;
        for (SpdySocket socket: sockets.values()) {
            org.bigleg.async.Util.writable(socket);
        }
    }
    ...
    public void goAway(int lastGoodStreamId, ErrorCode errorCode, ByteString debugData) {
        ...
        for (Iterator<Map.Entry<Integer, SpdySocket>> i = sockets.entrySet().iterator();
            ...
            if (streamId > lastGoodStreamId && entry.getValue().isLocallyInitiated()) {
                org.bigleg.async.Util.end(entry.getValue(), new IOException(ErrorCode.REFUSED_STREAM.toString()));
                i.remove();
            }
        }
    }
    ...
    public void error(Exception e) {
        ...
        for (Iterator<Map.Entry<Integer, SpdySocket>> i = sockets.entrySet().iterator(); i.hasNext();) {
            Map.Entry<Integer, SpdySocket> entry = i.next();
            org.bigleg.async.Util.end(entry.getValue(), e);
            i.remove();
        }
    }
}
